...
# ==========================================================
CROP_SIDE_TO_RL = "standard"  # ★ train_crop_3head.py와 동일하게 유지
...
def load_aux(stats_path):
    if os.path.exists(stats_path):
        ...
        # train_crop_3head.py가 저장한 crop_side_to_rl 값을 검증
...
def predict_patient_slices_crop(models, img_dir, slice_cols, device,
    ...
    for col in slice_cols:
        ...
        # ★ crop 기준: W//2  (train_crop_3head.py TBCTDatasetCrop.__getitem__과 동일)
        crops = {
            "left":  full_img[:, :w // 2],
            "right": full_img[:, w // 2:],
        }
...
def run_inference(best_model_path):
    ...
    print(f"\n추론 시작 (TTA={USE_TTA}, smooth={USE_SLICE_SMOOTH}, "
          f"{len(models)}모델 앙상블, crop 3-head 방식)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="TBCT Crop 3-head without-flip 추론 스크립트 (model_convNeXt-Tiny_crop_3head.py 전용)",
        usage="python3 inference_crop_3head.py <best_model_crop_3head_file.pth>",
    )
    parser.add_argument("best_model_path", type=str,
                        help="예: best_model_convNeXt-Tiny_crop_3head_fold0.pth")

    if len(sys.argv) < 2:
        print("\n  사용 예: python3 inference_crop_3head.py "
