"""
threshold 스윕 스크립트 (model_convNeXt-Tiny_crop_3head 전용)
...
목적:
    ...
    추론 로직은 inference_crop_3head.py의 함수를 그대로 import해서 사용하므로
    실제 추론과 100% 동일한 확률/스무딩이 쓰인다 (분포 어긋남 없음).

사용법:
    python3 sweep_thresholds_3head.py best_model_convNeXt-Tiny_crop_3head_fold0.pth

    ※ val_set.csv (정답 라벨)와 data/ 디렉토리가 있어야 함.
    ※ inference_crop_3head.py 가 같은 폴더에 있어야 함.
...
# [설정]
# ==========================================================
INFER_FILE = "inference_crop_3head.py"      # 추론 로직을 가져올 파일
...
# ==========================================================
# [inference_crop_3head.py 동적 import]
...
def load_infer_module(path):
    if not os.path.exists(path):
        ...
        sys.exit(1)
    spec = importlib.util.spec_from_file_location("infer3", path)
...
def run(best_model_path):
    ...
    print(f"  (참고) 원본 공유-head 점수 : 0.8641")
    print("=" * 70)

    if best_overall > 0.8641:
        print(">>> 단일 fold에서 원본(0.8641)을 넘었음. full 5-fold 진행 가치 충분.")
    else:
        print(">>> 아직 원본 미만. 단일 fold 노이즈일 수 있으니 GRID를 더 촘촘히")
        print("    (예: 0.02 간격) 보거나, full 5-fold로 재측정 후 판단할 것.")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("사용법: python3 sweep_thresholds_3head.py "
              "best_model_convNeXt-Tiny_crop_3head_fold0.pth")
        sys.exit(1)
    run(sys.argv[1])