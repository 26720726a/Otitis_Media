...
# ==========================================================
# [모델 파일 고정]
# (변경 a) 기존 2head와 체크포인트/stats가 충돌하지 않도록 _flip 모델 파일 사용
# ==========================================================
MODEL_FILE = "model_convNeXt-Tiny_crop_2head_flip.py"
...
#
# ★ train_crop_2head_flip.py / inference_crop_2head.py 양쪽에서 완전히 동일한 코드 사용.
...
# flip 증강 전 분포로 계산 (flip은 좌우 미러라 채널 mean/std 불변).
# ==========================================================
...
class TBCTDatasetCrop(Dataset):
    ...
    def _augment(self, img_3ch):
        """crop 이후 IMAGE_SIZE 정사각형 이미지에 적용. 좌우 flip 포함(학습 전용)."""
        h, w = img_3ch.shape[:2]

        # (변경 b) crop 한정 좌우 flip (학습 전용). 한쪽 귀만 담긴 crop이라
        # 뒤집어도 같은 귀의 거울상일 뿐 → 라벨 불변. otitis head를
        # 방향 불변으로 만들어 다수인 lt 양성이 rt 인식으로 전이되게 함.
        # ※ full-slice flip 금지 원칙과 무관 — 추론 TTA엔 flip 없음.
        if random.random() < 0.5:
            img_3ch = cv2.flip(img_3ch, 1)
...
# ==========================================================
def dump_crops(data_root, train_csv, n_patients=4, out_dir="./crop_2head_flip_debug"):
...
# ==========================================================
# [학습 메인 — K-Fold]
# (변경 c) max_folds 파라미터: 단일 fold 우선 검증용
# ==========================================================
def train(max_folds=None):
    DATA_ROOT = "./data"
    ...
    print(f"옵션: CLAHE={USE_CLAHE}, 2.5D={USE_25D}, FocalLoss={USE_FOCAL_LOSS}, "
          f"NUM_FOLDS={NUM_FOLDS}, NUM_TASKS={NUM_TASKS}(crop, 2-head + crop-flip 증강)")
    if max_folds is not None:
        print(f"[단일 fold 검증 모드] --folds {max_folds}: fold 0~{max_folds-1}만 학습")
    ...
    # ── mean/std: crop 적용 후 기준으로 1회 계산 (flip 전 분포) ──
    print("데이터셋 mean/std 계산 중 (crop 적용 기준, flip 전)...")
...
def train_fullfit(fullfit_epochs):
    ...
    print(f"옵션: CLAHE={USE_CLAHE}, 2.5D={USE_25D}, FocalLoss={USE_FOCAL_LOSS}, "
          f"drop_last=True, NUM_TASKS={NUM_TASKS}(crop, 2-head + crop-flip 증강)")
...
# ==========================================================
# [검증 절차]
# 1) python3 train_crop_2head_flip.py --folds 1        # fold0만 학습
# 2) python3 inference_crop_2head.py best_model_convNeXt-Tiny_crop_2head_flip_fold0.pth
# 3) python3 evaluation_per_task.py submission_validation.csv
#    → rt_otitis의 Otitis recall(기준 0.17) 확인
# 4) rt_otitis 오르고 lt_otitis·temporal 유지되면 --folds 없이 4-fold 전체 재학습 →
#    sweep로 threshold 재확정
# ==========================================================
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="TBCT Crop 2-head + crop-flip 증강 학습 스크립트 "
                    "(model_convNeXt-Tiny_crop_2head_flip.py 전용)",
        usage="python3 train_crop_2head_flip.py [--folds N] [--fullfit] "
              "[--fullfit-epochs N] [--dump-crops]",
    )
    ...
    parser.add_argument("--dump-crops", action="store_true",
                        help="crop 디버그 이미지를 ./crop_2head_flip_debug/에 저장 후 종료")
    # (변경 c) 단일 fold 우선 검증용
    parser.add_argument("--folds", type=int, default=None,
                        help="학습할 fold 수 (기본=전체). 예: --folds 1 이면 fold0만 학습")
