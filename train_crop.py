...
# [모델 파일 고정]
# ==========================================================
MODEL_FILE = "model_convNeXt-Tiny_crop.py"
...
def build_sample_list_crop(csv_path, base_path):
    ...
    if CROP_SIDE_TO_RL == "standard":
        left_t,  left_o  = 0, 2   # LEFT  crop → 환자 Rt
        right_t, right_o = 1, 3   # RIGHT crop → 환자 Lt
    else:
    ...
    for info in raw_dict.values():
        ...
        left_labels  = [lbl4[left_t],  lbl4[left_o]]
        right_labels = [lbl4[right_t], lbl4[right_o]]

        # 두 라벨이 모두 IGNORE인 경우 샘플 생성하지 않음
...
def compute_multi_loss(outputs, labels, criterion):
    ...
    for t in range(NUM_TASKS):
        ...
        if mask.sum() == 0:
            continue
        total_loss += criterion(outputs[mask, t, :], labels[mask, t])
        valid_tasks += 1
...
# ==========================================================
def dump_crops(data_root, train_csv, n_patients=4, out_dir="./crop_debug"):
...
def train():
    ...
    print(f"옵션: CLAHE={USE_CLAHE}, 2.5D={USE_25D}, FocalLoss={USE_FOCAL_LOSS}, "
          f"NUM_FOLDS={NUM_FOLDS}, NUM_TASKS={NUM_TASKS}(crop)")
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(fold_splits):
        ...
        if USE_FOCAL_LOSS:
            ...
            print("Loss: FocalLoss(gamma=2.0) × 2 heads (masked)")
        else:
            criterion = nn.CrossEntropyLoss(label_smoothing=0.05)
            print("Loss: CrossEntropyLoss × 2 heads (masked)")
...
def train_fullfit(fullfit_epochs):
    ...
    print(f"옵션: CLAHE={USE_CLAHE}, 2.5D={USE_25D}, FocalLoss={USE_FOCAL_LOSS}, "
          f"drop_last=True, NUM_TASKS={NUM_TASKS}(crop)")
    ...
    all_labels_flat = [l for s in all_samples for l in s["labels"] if l >= 0]
    cls_count = np.bincount(all_labels_flat, minlength=2)
    pos_ratio = cls_count[1] / max(cls_count.sum(), 1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="TBCT Crop 방식 학습 스크립트 (model_convNeXt-Tiny_crop.py 전용)",
        usage="python3 train_crop.py [--fullfit] [--fullfit-epochs N] [--dump-crops]",
    )
    ...
    parser.add_argument("--dump-crops", action="store_true",
                        help="crop 디버그 이미지를 ./crop_debug/에 저장 후 종료 (학습 안 함)")
