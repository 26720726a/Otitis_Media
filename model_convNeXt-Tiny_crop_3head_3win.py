...
#
# ★ 3-head 3-window 입력 실험 전용 격리 파일
#   입력 채널: [bone window(PNG_bone), soft window(PNG_soft), 원본 PNG]
#   train_crop_3head_3win.py / inference_crop_3head_3win.py 전용으로 사용.
#   체크포인트: best_model_convNeXt-Tiny_crop_3head_3win_fold{k}.pth
#   stats:      preproc_model_convNeXt-Tiny_crop_3head_3win_stats.json
# ==========================================================
NUM_TASKS  = 3
...
class MyModel(nn.Module):
    """
    3-Head ConvNeXt-Tiny (crop 입력 전용) — 3-window 입력 실험용
    ...
    입력 3채널이 2.5D(prev/cur/next) 대신 bone/soft/원본 window로 교체됨.
    채널0=PNG_bone(700/4000), 채널1=PNG_soft(35/250), 채널2=PNG(원본).
    ...
    def forward(self, x):
        """
        Args:
            x: (B, 3, H, W) — 한쪽 귀 crop, 채널: [bone, soft, png]
        Returns:
