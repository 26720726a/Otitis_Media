...
SMOOTH_WINDOW      = 3
SOFT_WINDOW        = (35, 250)  # 학습 PNG_soft 생성에 쓰인 window

# crop 2-head 모델의 출력 태스크
CROP_TASK_NAMES = ["temporal", "otitis"]   # head index 0, 1
...
# ==========================================================
# [★ 좌우 crop 방향 설정 — train_crop_2head.py와 반드시 일치시킬 것]
#
# train_crop_2head.py의 CROP_SIDE_TO_RL과 이 값이 다르면
...
# ==========================================================
CROP_SIDE_TO_RL = "standard"  # ★ train_crop_2head.py와 동일하게 유지
...
# head_idx: 0=temporal, 1=otitis  (rt/lt crop이 otitis head를 공유)
#
# standard 기준:
#   rt_temporal → LEFT  crop, temporal head(0)
#   lt_temporal → RIGHT crop, temporal head(0)
#   rt_otitis   → LEFT  crop, otitis head(1)   ← 2-head: head_idx=1 (공유)
#   lt_otitis   → RIGHT crop, otitis head(1)   ← 2-head: head_idx=1 (공유)
#
# ★ crop 기준 W//2 도 train_crop_2head.py와 동일. 변경 금지.
# ==========================================================
if CROP_SIDE_TO_RL == "standard":
    TASK_TO_CROP = {
        ...
        2: ("left",  1),   # rt_otitis   → left  crop, otitis head (1)
        3: ("right", 1),   # lt_otitis   → right crop, otitis head (1)
    }
else:  # "flipped"
    TASK_TO_CROP = {
        0: ("right", 0),   # rt_temporal → right crop, temporal head (0)
        1: ("left",  0),   # lt_temporal → left  crop, temporal head (0)
        2: ("right", 1),   # rt_otitis   → right crop, otitis head (1)
        3: ("left",  1),   # lt_otitis   → left  crop, otitis head (1)
    }
...
def load_aux(stats_path):
    if os.path.exists(stats_path):
        ...
        # train_crop_2head.py가 저장한 crop_side_to_rl 값을 검증
...
def dcm_to_soft_uint8(dcm_path, center=35, width=250):
    try:
        import pydicom
    except ImportError as e:
        raise ImportError(
            "DICOM 직접 추론에는 pydicom이 필요합니다. "
            "pydicom을 설치하거나 PNG_soft 폴더를 사용하세요."
        ) from e
...
def resolve_data_root(data_root_arg=None):
    candidates = []
    if data_root_arg:
        candidates.append(data_root_arg)
    ...
    raise FileNotFoundError(f"data 디렉토리를 찾을 수 없습니다. 확인한 후보: {checked}")


def _list_dcm_files(dcm_dir):
    if not os.path.isdir(dcm_dir):
        return []
    return [
        os.path.join(dcm_dir, name)
        for name in os.listdir(dcm_dir)
        if name.lower().endswith(".dcm")
    ]


def _build_dicom_source(pid, dcm_dir):
    files = sorted(_list_dcm_files(dcm_dir))
    name_index = {os.path.basename(path).lower(): path for path in files}
    direct_map = {}
    for n in range(1, 133):
        for stem in (f"{n:04d}", f"{n}", f"{n:03d}", f"{n:05d}"):
            path = name_index.get(f"{stem}.dcm")
            if path is not None:
                direct_map[n] = path
                break
    ...
    for path in files:
        try:
            ...
            if inst is not None:
                instance_map[int(inst)] = path
        except Exception as e:
            print(f"[경고] pid={pid}: DICOM InstanceNumber 읽기 실패: {path} ({e})")

    slice_map = {}
    for n in range(1, 133):
        slice_map[n] = direct_map.get(n) or instance_map.get(n)

    mapped_count = sum(1 for path in slice_map.values() if path is not None)
    return {
        "kind": "DICOM",
        "pid": pid,
        "dir": dcm_dir,
...
def resolve_patient_source(pid, data_root):
    base_candidates = [
        os.path.join(data_root, pid),
        os.path.join(data_root, "val", pid),
        os.path.join(data_root, "train", pid),
    ]
    ...
    for base in base_candidates:
        ...
        png_dir = os.path.join(base, "PNG_soft")
        if os.path.isdir(png_dir):
            mapped_count = len(glob.glob(os.path.join(png_dir, "*.png")))
            source = {
                "kind": "PNG_soft",
                "pid": pid,
                "dir": png_dir,
            ...
            print(f"[소스] pid={pid}: PNG_soft 사용 | mapped_slices={mapped_count} | {png_dir}")
            return source

        for subdir in ("DCM", "dcm", "DICOM", "dicom", ""):
            dcm_dir = os.path.join(base, subdir) if subdir else base
            if _list_dcm_files(dcm_dir):
                source = _build_dicom_source(pid, dcm_dir)
                print(f"[소스] pid={pid}: DICOM 사용 | mapped_slices={source['mapped_count']} | {dcm_dir}")
                return source

    print(f"[경고] pid={pid}: PNG_soft도 DICOM도 못 찾음 → 해당 환자 0으로 남음")
    return {"kind": "missing", "pid": pid, "mapped_count": 0}


def get_source_path(source, n):
    if source["kind"] == "PNG_soft":
        path = os.path.join(source["dir"], f"{n:04d}.png")
        return path if os.path.exists(path) else None
    if source["kind"] == "DICOM":
        return source["slice_map"].get(n)
    return None


def load_windowed_uint8(source, n):
    path = get_source_path(source, n)
    if path is None:
        return None
    ...
    if source["kind"] == "DICOM":
        return dcm_to_soft_uint8(path, *SOFT_WINDOW)
    return None


def load_gray(source, n, use_clahe):
    img = load_windowed_uint8(source, n)
    if img is None:
        return None
    if use_clahe:
...
def load_25d(source, n, use_clahe, use_25d):
    cur_path = get_source_path(source, n)
    if cur_path is None:
        return None

    cur = load_gray(source, n, use_clahe)
    if cur is None:
        return None
    ...
    prev_path = get_source_path(source, n - 1) if n > 1 else None
    next_path = get_source_path(source, n + 1) if n < 132 else None

    prev = load_gray(source, n - 1, use_clahe) if prev_path else cur
    nxt  = load_gray(source, n + 1, use_clahe) if next_path else cur
...
# ==========================================================
# [환자 1명 전체 슬라이스 예측 — crop 2-head 버전]
#
# 각 슬라이스에 대해:
#   ① full 2.5D 이미지 로드
#   ② 가로 W//2 기준으로 left / right crop 분리
#      ★ 이 W//2 기준이 train_crop_2head.py의 __getitem__과 동일해야 함
#   ③ 각 crop에 TTA + 폴드 앙상블 적용
#   ④ 결과: {"left": (2,), "right": (2,)} — 각 (temporal, otitis) 확률
#      head_idx 0=temporal, 1=otitis (rt/lt crop 공유)
# ==========================================================
@torch.no_grad()
def predict_patient_slices_crop(models, source, slice_cols, device,
                                image_size, mean, std, use_clahe, use_25d):
    results = {}   # slice_n → {"left": np.array(2,), "right": np.array(2,)}

    for col in slice_cols:
        n        = int(col)
        full_img = load_25d(source, n, use_clahe, use_25d)
        if full_img is None:
            continue

        h, w = full_img.shape[:2]

        # ★ crop 기준: W//2  (train_crop_2head.py TBCTDatasetCrop.__getitem__과 동일)
        crops = {
            "left":  full_img[:, :w // 2],
            "right": full_img[:, w // 2:],
        }
        ...
        for side, crop_img in crops.items():
            ...
            fold_probs = []
            for m in models:
                outputs = m(batch)                                  # (V, 2, 2)
                probs   = torch.softmax(outputs, dim=2)[:, :, 1]   # (V, 2)
                fold_probs.append(probs.mean(dim=0))                # (2,)

            avg = torch.stack(fold_probs, dim=0).mean(dim=0).cpu().numpy()  # (2,)
...
# [추론 메인]
# ==========================================================
def run_inference(best_model_path, data_root_arg=None):
    DATA_ROOT = resolve_data_root(data_root_arg)
    ...
    print(f"\n추론 시작 (TTA={USE_TTA}, smooth={USE_SLICE_SMOOTH}, "
          f"{len(models)}모델 앙상블, crop 2-head 방식)")
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if p_id not in prediction_cache:
            source = source_cache.get(p_id)
            if source is None:
                source = resolve_patient_source(p_id, DATA_ROOT)
                source_cache[p_id] = source

            if source["kind"] != "missing":
                prediction_cache[p_id] = predict_patient_slices_crop(
                    models, source, slice_cols, device,
                    image_size, mean, std, use_clahe, use_25d,
                )
            else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="TBCT Crop 2-head shared-otitis 추론 스크립트 (model_convNeXt-Tiny_crop_2head.py 전용)",
        usage="python3 inference_crop_2head.py <best_model_crop_2head_file.pth>",
    )
    parser.add_argument("best_model_path", type=str,
                        help="예: best_model_convNeXt-Tiny_crop_2head_fold0.pth")
    parser.add_argument("--data-root", type=str, default=None,
                        help="data 루트 직접 지정 (기본 후보: ./data → ../data)")

    if len(sys.argv) < 2:
        print("\n  사용 예: python3 inference_crop_2head.py "
              "best_model_convNeXt-Tiny_crop_2head_fold0.pth\n")
        sys.exit(1)

    args = parser.parse_args()
    run_inference(args.best_model_path, args.data_root)
