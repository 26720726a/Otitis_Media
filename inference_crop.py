...
# crop 모델의 출력 태스크
CROP_TASK_NAMES = ["temporal", "otitis"]   # index 0, 1
...
# ==========================================================
# [★ 좌우 crop 방향 설정 — train_crop.py와 반드시 일치시킬 것]
#
# train_crop.py의 CROP_SIDE_TO_RL과 이 값이 다르면
...
# ==========================================================
CROP_SIDE_TO_RL = "standard"  # ★ train_crop.py와 동일하게 유지


# ==========================================================
# [4-head task_idx → (crop_side, crop_task_idx) 매핑]
#
# task_idx: 0=rt_temporal, 1=lt_temporal, 2=rt_otitis, 3=lt_otitis
# crop_task_idx: 0=temporal, 1=otitis
#
# ★ crop 기준 W//2 도 train_crop.py와 동일. 변경 금지.
# ==========================================================
if CROP_SIDE_TO_RL == "standard":
    TASK_TO_CROP = {
        0: ("left",  0),   # rt_temporal → left  crop, temporal
        1: ("right", 0),   # lt_temporal → right crop, temporal
        2: ("left",  1),   # rt_otitis   → left  crop, otitis
        3: ("right", 1),   # lt_otitis   → right crop, otitis
    }
else:  # "flipped"
    TASK_TO_CROP = {
        0: ("right", 0),   # rt_temporal → right crop, temporal
        1: ("left",  0),   # lt_temporal → left  crop, temporal
        2: ("right", 1),   # rt_otitis   → right crop, otitis
        3: ("left",  1),   # lt_otitis   → left  crop, otitis
    }
...
# rt_otitis / lt_otitis의 양성 비율이 다르면 값을 다르게 줄 수 있습니다.
#   rt_otitis: 양성 소수 클래스 → 낮은 threshold로 recall 확보
#   lt_otitis: 양성 다수 클래스 → 높은 threshold로 precision 유지
# ==========================================================
MANUAL_THRESHOLDS = {
    "rt_temporal": 0.55,
    "lt_temporal": 0.55,
...
def load_aux(stats_path):
    if os.path.exists(stats_path):
        ...
        # train_crop.py가 저장한 crop_side_to_rl 값을 검증
...
#
# ★ train_crop.py / inference_crop.py 양쪽에서 완전히 동일한 코드 사용.
...
# ==========================================================
# [환자 1명 전체 슬라이스 예측 — crop 버전]
#
# 각 슬라이스에 대해:
#   ① full 2.5D 이미지 로드
#   ② 가로 W//2 기준으로 left / right crop 분리
#      ★ 이 W//2 기준이 train_crop.py의 __getitem__과 동일해야 함
...
def predict_patient_slices_crop(models, img_dir, slice_cols, device,
    ...
    for col in slice_cols:
        ...
        # ★ crop 기준: W//2  (train_crop.py TBCTDatasetCrop.__getitem__과 동일)
        crops = {
            "left":  full_img[:, :w // 2],
            "right": full_img[:, w // 2:],
        }
...
def run_inference(best_model_path):
    ...
    print(f"\n추론 시작 (TTA={USE_TTA}, smooth={USE_SLICE_SMOOTH}, "
          f"{len(models)}모델 앙상블, crop 방식)")

    # ── 환자별 슬라이스 예측 캐시 ──
    # prediction_cache[pid][slice_n] = {"left": (2,), "right": (2,)}
    prediction_cache = {}

    for idx, row in tqdm(df.iterrows(), total=len(df)):
        p_id     = str(int(row["No"]))
        task_idx = get_task_index(row["R/L"], row["Image number"])
        side, crop_t = TASK_TO_CROP[task_idx]
        ...
        # 해당 (side, crop_t)의 슬라이스별 확률 시퀀스 추출
        task_probs = []
        for col in slice_cols:
            n = int(col)
            if n in patient_preds:
                task_probs.append(float(patient_preds[n][side][crop_t]))
            else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="TBCT Crop 방식 추론 스크립트 (model_convNeXt-Tiny_crop.py 전용)",
        usage="python3 inference_crop.py <best_model_crop_file.pth>",
    )
    parser.add_argument("best_model_path", type=str,
                        help="예: best_model_convNeXt-Tiny_crop_fold0.pth")

    if len(sys.argv) < 2:
        print("\n  사용 예: python3 inference_crop.py "
              "best_model_convNeXt-Tiny_crop_fold0.pth\n")
        sys.exit(1)
