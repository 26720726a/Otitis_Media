...
# [모델 파일 고정]
# ==========================================================
MODEL_FILE = "model_convNeXt-Tiny_crop_3head_3win.py"
...
USE_CLAHE      = True
USE_25D        = False   # 3-window 모드: 2.5D 비활성화
USE_FOCAL_LOSS = True

# ==========================================================
# [3-window 입력 설정]
#
# 2.5D(prev/cur/next) 대신 같은 cur 슬라이스를 3개 window로 현상한 이미지를 사용.
#   채널0 = PNG_bone  (bone window, 700/4000)  → temporal 뼈 구조에 유리
#   채널1 = PNG_soft  (soft window, 35/250)    → otitis 액체/조직 대비에 유리
#   채널2 = PNG       (원본)                    → 기존 best 모델이 쓰던 정보
#
# ★ WIN_SUBDIRS 순서 = 채널 순서. 변경 시 inference_crop_3head_3win.py도 동일하게 변경.
# ★ PNG_bone, PNG_soft 폴더는 각 환자 폴더 하위에 PNG와 같은 레벨로 존재 가정.
#    파일명 규칙은 PNG와 동일: {n:04d}.png
#    해당 window 파일이 없으면 원본 PNG로 폴백 (조용히).
# ==========================================================
USE_3WINDOW  = True
...
# ==========================================================
# [샘플 리스트 구축 — 좌/우 crop 분리, 3-slot 라벨]
#
#   1단계: (환자, 슬라이스)마다 4개 라벨 수집
#          [rt_temporal, lt_temporal, rt_otitis, lt_otitis]
#   2단계: CROP_SIDE_TO_RL에 따라 좌/우 2개 샘플로 분리
#          각 crop 샘플 라벨: [temporal, rt_otitis, lt_otitis] (shape=(3,))
#            - LEFT  crop (standard=환자Rt): [rt_temp, rt_otitis, IGNORE]
#            - RIGHT crop (standard=환자Lt): [lt_temp, IGNORE,   lt_otitis]
#
#   flip 없음. otitis head 분리는 라벨 라우팅으로만 수행.
# ==========================================================
def build_sample_list_crop(csv_path, base_path):
    ...
    # ── 2단계: 좌/우 crop 샘플 분리 (3-slot 라벨) ──
    # 슬롯: [0]=temporal, [1]=rt_otitis, [2]=lt_otitis
    # labels_4 인덱스: rt_temporal=0, lt_temporal=1, rt_otitis=2, lt_otitis=3
    if CROP_SIDE_TO_RL == "standard":
        left_t,  left_o,  left_ot_slot  = 0, 2, 1   # LEFT=환자Rt  → rt_otitis 슬롯(1)
        right_t, right_o, right_ot_slot = 1, 3, 2   # RIGHT=환자Lt → lt_otitis 슬롯(2)
    else:
        left_t,  left_o,  left_ot_slot  = 1, 3, 2   # LEFT=환자Lt  → lt_otitis 슬롯(2)
        right_t, right_o, right_ot_slot = 0, 2, 1   # RIGHT=환자Rt → rt_otitis 슬롯(1)
    ...
    for info in raw_dict.values():
        ...
        left_labels  = [IGNORE_LABEL, IGNORE_LABEL, IGNORE_LABEL]
        left_labels[0]            = lbl4[left_t]
        left_labels[left_ot_slot] = lbl4[left_o]

        right_labels = [IGNORE_LABEL, IGNORE_LABEL, IGNORE_LABEL]
        right_labels[0]             = lbl4[right_t]
        right_labels[right_ot_slot] = lbl4[right_o]
...
class TBCTDatasetCrop(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels = torch.tensor(s["labels"], dtype=torch.long)  # (3,)
...
# ==========================================================
# [Multi-Head Loss — 3-head, head별 독립 criterion]
#
#   outputs:  (B, 3, 2)
#   labels:   (B, 3)    (-1이면 무시)
#   criteria: list of 3 loss functions, criteria[t] for head t
# ==========================================================
...
def collect_preds_prob(model, loader):
    ...
    for x, y in loader:
        ...
        outputs = model(x)                              # (B, 3, 2)
        probs   = torch.softmax(outputs, dim=2)[:, :, 1]  # (B, 3)
...
# ==========================================================
def dump_crops(data_root, train_csv, n_patients=4, out_dir="./crop_3head_3win_debug"):
...
def train():
    ...
    print(f"옵션: CLAHE={USE_CLAHE}, 2.5D={USE_25D}, FocalLoss={USE_FOCAL_LOSS}, "
          f"NUM_FOLDS={NUM_FOLDS}, NUM_TASKS={NUM_TASKS}(crop, 3-head without-flip)")
    ...
    with open(preproc_stats_path, "w") as f:
        json.dump({
            ...
            "use_3window": USE_3WINDOW, "win_subdirs": WIN_SUBDIRS,
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(fold_splits):
        ...
        # ── head별 독립 criterion (class 분포가 head마다 다름) ──
        print("Loss: head별 FocalLoss(gamma=2.0) × 3 heads (masked)")
        criteria = []
        for t in range(NUM_TASKS):
...
def train_fullfit(fullfit_epochs):
    ...
    print(f"옵션: CLAHE={USE_CLAHE}, 2.5D={USE_25D}, FocalLoss={USE_FOCAL_LOSS}, "
          f"drop_last=True, NUM_TASKS={NUM_TASKS}(crop, 3-head without-flip)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="TBCT Crop 3-head 3-window 입력 학습 스크립트 (model_convNeXt-Tiny_crop_3head_3win.py 전용)",
        usage="python3 train_crop_3head_3win.py [--fullfit] [--fullfit-epochs N] [--dump-crops]",
    )
    ...
    parser.add_argument("--dump-crops", action="store_true",
                        help="crop 디버그 이미지를 ./crop_3head_3win_debug/에 저장 후 종료 (학습 안 함)")
